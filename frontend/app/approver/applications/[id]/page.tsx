...
export default function ApproverApplicationDetailPage() {
  ...
  const canAct = app?.status === "ACCEPTED";

  useEffect(() => {
    if (!identity) { router.replace("/"); return; }
    approverApi.get(identity, id)
      .then(setApp)
  ...
  async function handleAction(action: string, notes: string) {
    if (!identity) return;
    await approverApi.takeAction(identity, id, { action: action as ApproveAction, notes });
