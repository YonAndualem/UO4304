...
func TestTradeLicenseApplication_Submit_FailsWithoutDocuments(t *testing.T) {
	...
	app.SettlePayment(tradelivense.NewPayment(500, "USD", "TXN-001"))
...
func TestTradeLicenseApplication_Submit_FailsWithoutPayment(t *testing.T) {
	...
	err := app.Submit()
	if err != tradelivense.ErrPaymentRequired {
...
func TestTradeLicenseApplication_Cancel_HappyPath(t *testing.T) {
	...
	if app.Status != tradelivense.StatusCancelled {
...
func TestFullApprovalWorkflow(t *testing.T) {
	...
	steps := []struct {
		name   string
		action func() error
		want   tradelivense.ApplicationStatus
	}{
		{"submit", func() error { return app.Submit() }, tradelivense.StatusSubmitted},
		{"accept", func() error { return app.Accept("reviewer-1") }, tradelivense.StatusAccepted},
		{"approve", func() error { return app.Approve("approver-1") }, tradelivense.StatusApproved},
	}
